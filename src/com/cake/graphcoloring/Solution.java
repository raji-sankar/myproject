package com.cake.graphcoloring;
...
public class Solution {
    ...
    public static void colorGraph(GraphNode[] graph, String[] colors) {
        ...
        int degree = getDegree( graph);
        Set<GraphNode> seen = new HashSet<>();

        int colorIndex = -1;
        int maxColorIndex = degree;
        if(maxColorIndex > colors.length -1){
            throw new IllegalArgumentException("Need more colors");
        }
        ...
        for(GraphNode node: graph){
            if(!seen.contains(node)){
                ...
                node.setColor(colors[colorIndex]);
                seen.add(node);
                colorIndex++;
                colorIndex = colorIndex % (maxColorIndex + 1);
                for (GraphNode neighbor: node.getNeighbors()){
                    if(!seen.contains(neighbor))  {
                        neighbor.setColor(colors[colorIndex]);
                        seen.add(neighbor);
                    }
                }
    ...
    public static int getDegree(GraphNode[] graph){
        int degree = 0;

        for(GraphNode node: graph){
            degree = Math.max(degree, node.getNeighbors().size());
        }
        return degree;
    }
